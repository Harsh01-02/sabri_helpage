...
const HomePage = ({ onNavigate }) => {
  ...
  const getSection = (type) => {
    return pageData?.sections?.find(section => section.type === type);
  };
  ...
  const causesSection = getSection('causes');
  const storiesSection = getSection('stories');
  const videoSection = getSection('video');
  const supportersSection = getSection('supporters');
  ...
  return (
    <>
      ...
      {heroSection && (
        <section className="relative h-[600px] md:h-[700px] flex items-center justify-center text-center overflow-hidden bg-black">
          <div className="absolute inset-0">
            <ResponsiveImage
              src={heroSection.backgroundImage || IMAGE_URLS.HERO}
              alt="Hero Background"
          ...
          <div className="relative z-10 max-w-5xl mx-auto px-4 animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {heroSection.cta && heroSection.cta.map((button, idx) => (
                <button
                  key={idx}
                  onClick={() => onNavigate(button.targetSlug)}
                  className={`px-10 py-4 rounded-full font-bold text-lg transition transform hover:scale-105 shadow-lg ${
                    button.variant === 'primary' 
                      ? 'text-white' 
                      : 'bg-white text-black hover:bg-gray-100 border-2 border-white'
                  }`}
                  style={button.variant === 'primary' ? { backgroundColor: COLORS.PRIMARY } : {}}
                >
                  {button.label}
                </button>
              ))}
          ...
          {heroSection.subhead && (
            <div className="absolute bottom-0 left-0 w-full py-3 text-center group cursor-default z-20 transition-all duration-300" style={{ backgroundColor: COLORS.PRIMARY }}>
              ...
              <span className="text-white text-xl md:text-2xl font-medium hidden group-hover:block tracking-wide">
                {heroSection.subheadTranslation || 'May all be happy'}
              </span>
            </div>
          )}
        </section>
      ...
      {/* Stats & Video Section - Fully Dynamic */}
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Stats Card - Fully Dynamic */}
            {statsSection && statsSection.items && (
              <div className="flex flex-col h-full">
                ...
                >
                  <p className="text-sm font-bold uppercase tracking-widest text-white/90 mb-4">
                    {statsSection.title || 'Our Impact in Numbers'}
                  </p>

                  <h3 className="text-3xl sm:text-4xl font-extrabold text-white mb-8 leading-tight">
                    {statsSection.subtitle || 'Making a Difference, One Life at a Time'}
                  </h3>
              ...
              <div className="flex flex-col gap-6 lg:pl-12">
                <div className="bg-white p-3 rounded-3xl shadow-xl border border-gray-100 transform transition-all hover:shadow-2xl">
                  <div className="relative w-full overflow-hidden rounded-2xl aspect-video bg-black/5">
                    <YoutubeEmbed videoId={videoSection.videoId} title={videoSection.title} />
                  </div>
                </div>

                {videoSection.channelLink && (
                  <a 
                    href={videoSection.channelLink} 
                    target="_blank" 
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                          {videoSection.channelLabel || 'Watch Our Stories'}
                        </span>
      ...
      {/* Causes Section - Fully Dynamic */}
      {causesSection && causesSection.items && (
        <section className="py-20" style={{ backgroundColor: COLORS.BG_LIGHT_GRAY }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <SectionTitle className="mb-16">
              {causesSection.title || 'Our Core Causes'}
            </SectionTitle>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {causesSection.items.map((cause, idx) => (
                <div key={idx} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-500 transform hover:-translate-y-2">
                  <div className="h-56 relative">
                    <ResponsiveImage src={cause.image} className="w-full h-full object-cover" alt={cause.title} />
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex gap-3">
                      <button 
                        onClick={() => onNavigate(cause.route || cause.title.toLowerCase().replace(/\s+/g, '-'))} 
      ...
      {/* Supporters Section - Fully Dynamic */}
      {supportersSection && supportersSection.items && (
        <section className="py-12 bg-white border-y border-gray-200">
          <div className="max-w-7xl mx-auto px-4 text-center">
            <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">
              {supportersSection.title || 'Our Supporters'}
            </p>
            <div className="flex flex-wrap justify-center gap-8 md:gap-16 items-center opacity-60 grayscale hover:grayscale-0 transition duration-500">
              {supportersSection.items.map((supporter, i) => (
      ...
      {/* Stories Section - Fully Dynamic */}
      {storiesSection && storiesSection.items && (
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="relative flex items-center justify-center mb-12">
              <SectionTitle className="text-center">
                {storiesSection.title || 'Our Stories'}
              </SectionTitle>
              {storiesSection.ctaLabel && storiesSection.ctaSlug && (
                <button 
                  onClick={() => onNavigate(storiesSection.ctaSlug)}
                  className="hidden md:flex absolute right-0 items-center font-bold"
                  style={{ color: COLORS.PRIMARY }}
                >
                  {storiesSection.ctaLabel} <ArrowRight className="ml-2 w-5 h-5" />
                </button>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {storiesSection.items.map((story, idx) => (
                <div key={idx} className="group cursor-pointer" onClick={() => onNavigate(story.slug || 'blog')}>
            ...
            {storiesSection.ctaLabel && storiesSection.ctaSlug && (
              <div className="mt-10 text-center md:hidden">
                ...
                >
                  {storiesSection.ctaLabel} Stories
                </button>
              </div>
            )}
          </div>
