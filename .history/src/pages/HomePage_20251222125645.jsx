...
const HomePage = ({ onNavigate }) => {
  const pageData = usePagesStore((state)=>state.getPageBySlug("home")) // this line gets the data for home page from the global store
  React.useEffect(() => {
    if (pageData) {
      console.log("Home page data:", pageData); // currently just printing this data hence its visible on console
    }
  }, [pageData]);
  ...
  if (!pageData) {
    ...
    // Dynamically find sections by type
    const heroSection = pageData.sections?.find(s => s.type === 'hero') || {};
    const statsSection = pageData.sections?.find(s => s.type === 'stats') || {};
    const videoSection = pageData.sections?.find(s => s.type === 'video') || {};
    const causesSection = pageData.sections?.find(s => s.type === 'causes') || {};
  ...
  return (
    <>
      ...
      {/* What We Stand For Section (updated, before milestones/impact) */}
      ...
      <section className="py-20 bg-gradient-to-b from-white via-gray-50 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12 text-center">
            <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-3 tracking-tight">Milestones & Impact</h2>
            <p className="text-lg text-gray-500">A snapshot of our journey and the lives we’ve touched</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
            {(statsSection.items || []).map((stat, idx) => {
              // Pick an icon for each stat (fallback: Star)
              const icons = [
                <Star className="w-8 h-8 text-yellow-400 mb-2" key="icon1" />,
                <Users className="w-8 h-8 text-blue-500 mb-2" key="icon2" />,
                <Heart className="w-8 h-8 text-red-400 mb-2" key="icon3" />,
                <CheckCircle className="w-8 h-8 text-green-500 mb-2" key="icon4" />
              ];
              const icon = icons[idx % icons.length];
              return (
                <div
                  key={idx}
                  className="bg-white/80 shadow-lg border border-gray-100 rounded-2xl px-8 py-10 flex flex-col items-center hover:shadow-2xl transition-all duration-300 group relative overflow-hidden"
                  style={{ minHeight: 180 }}
                >
                  ...
                  <div className="z-10 flex flex-col items-center">
                    {icon}
                    <span className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-2 tracking-tight group-hover:text-primary-600 transition-colors">{stat.value}</span>
                    <span className="block text-base font-medium text-gray-600 text-center">{stat.label}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div className="bg-white p-4 md:p-8 rounded-3xl shadow-xl border border-gray-100 transform transition-all hover:shadow-2xl">
            ...
            <a
              ...
              className="group flex items-center justify-between p-6 rounded-2xl bg-gray-50 hover:bg-white border border-gray-200 hover:border-red-100 hover:shadow-lg transition-all duration-300 cursor-pointer"
            >
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 flex items-center justify-center bg-white rounded-full shadow-sm group-hover:scale-110 transition-transform duration-300 border border-gray-100">
                ...
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Watch Our Stories</span>
