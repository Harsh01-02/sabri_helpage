    ...
    <>
      ...
      <section className="relative h-[600px] md:h-[700px] flex items-center justify-center text-center overflow-hidden bg-black">
        <div className="absolute inset-0">
          <ResponsiveImage
            src={pageData.sections[0].heroImage || IMAGE_URLS.HERO}
            alt="Hero Background"
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 animate-fade-in-up">
          <h1 className="text-5xl md:text-7xl font-extrabold leading-tight mb-6 tracking-tight" style={{ color: COLORS.PRIMARY }}>
            {pageData.sections[0].headline}
          </h1>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={() => onNavigate(pageData.sections[0].primaryButtonRoute || 'about')}
              className="text-white px-10 py-4 rounded-full font-bold text-lg transition transform hover:scale-105 shadow-lg"
              style={{ backgroundColor: COLORS.PRIMARY }}
            >
              {pageData.sections[0].primaryButtonText || 'Read More'}
            </button>
            <button
              onClick={() => onNavigate(pageData.sections[0].secondaryButtonRoute || 'donate')}
            ...
            >
              {pageData.sections[0].secondaryButtonText || 'Support Us'}
            </button>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 w-full py-3 text-center group cursor-default z-20 transition-all duration-300" style={{ backgroundColor: COLORS.PRIMARY }}>
             <span className="text-white text-xl md:text-2xl font-medium block group-hover:hidden tracking-wide">
               {pageData.sections[0].sanskritText || 'सर्वे भवन्तु सुखिनः'}
             </span>
             <span className="text-white text-xl md:text-2xl font-medium hidden group-hover:block tracking-wide">
               {pageData.sections[0].sanskritTranslation || 'May all be happy'}
             </span>
        </div>
      </section>

      {/* Impact Stats Section - sections[1] */}
            ...
            <div className="flex flex-col h-full">
              ...
              >
                <p className="text-sm font-bold uppercase tracking-widest text-white/90 mb-4">
                  {pageData.sections[1].subtitle || 'Our Impact in Numbers'}
                </p>

                <h3 className="text-3xl sm:text-4xl font-extrabold text-white mb-8 leading-tight">
                  {pageData.sections[1].title || 'Making a Difference, One Life at a Time'}
                </h3>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-auto">
                  {pageData.sections[1].items.map((stat, idx) => (
                    <div 
                      key={idx} 
            ...
            <div className="flex flex-col gap-6 lg:pl-12">
              <div className="bg-white p-3 rounded-3xl shadow-xl border border-gray-100 transform transition-all hover:shadow-2xl">
                <div className="relative w-full overflow-hidden rounded-2xl aspect-video bg-black/5">
                   <YoutubeEmbed 
                     videoId={pageData.sections[1].youtubeVideoId || YOUTUBE_VIDEO_ID} 
                     title={pageData.sections[1].videoTitle || "Sabri Helpage Story"} 
                   />
                </div>
              </div>

              <a 
                href={pageData.sections[1].youtubeChannelUrl || "https://www.youtube.com/@sabrihelpage167"} 
                target="_blank" 
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                      {pageData.sections[1].channelLabel || 'Watch Our Stories'}
                    </span>
                    <span className="text-base font-bold text-gray-900 group-hover:text-red-600 transition-colors">
                      {pageData.sections[1].channelButtonText || 'Visit YouTube Channel'}
                    </span>
                  </div>
                </div>
      ...
      {/* Core Causes Section - sections[2] */}
      <section className="py-20" style={{ backgroundColor: COLORS.BG_LIGHT_GRAY }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionTitle className="mb-16">{pageData.sections[2].title || 'Our Core Causes'}</SectionTitle>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pageData.sections[2].items.map((cause, idx) => (
              <div key={idx} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-500 transform hover:-translate-y-2">
                ...
                <div className="p-6">
                  <p className="text-gray-600 mb-6 text-sm line-clamp-3">
                    {cause.description || `Dedicated to providing resources, support, and advocacy for ${cause.title.toLowerCase()} initiatives across the country.`}
                  </p>
      ...
      <section className="py-12 bg-white border-y border-gray-200">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 items-center opacity-60 grayscale hover:grayscale-0 transition duration-500">
            {pageData.sections[3].items.map((supporter, i) => (
              <span key={i} className="text-xl md:text-2xl font-bold text-gray-600">
                {supporter.name}
              </span>
            ))}
          </div>
      ...
      {/* Testimonials Section - sections[4] */}
      <TestimonialsSection />

      {/* Stories/News Section - sections[5] */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative flex items-center justify-center mb-12">
            <SectionTitle className="text-center">
              {pageData.sections[5].title || 'Our Stories'}
            </SectionTitle>
            <button 
              onClick={() => onNavigate(pageData.sections[5].viewAllRoute || 'stories')}
              className="hidden md:flex absolute right-0 items-center font-bold"
              style={{ color: COLORS.PRIMARY }}
            >
              {pageData.sections[5].viewAllText || 'Read All'} <ArrowRight className="ml-2 w-5 h-5" />
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {pageData.sections[5].items.map((story, idx) => (
              <div key={idx} className="group cursor-pointer" onClick={() => onNavigate(story.route || 'blog')}>
                ...
                <h3 className="text-xl font-bold text-gray-900 mt-2 transition" style={{ color: COLORS.TEXT_DARK }}>
                  {story.title}
                </h3>
                <p className="text-gray-600 mt-2 text-sm line-clamp-2">
                  {story.snippet}
                </p>
                <button className="mt-4 text-sm font-bold underline decoration-2 underline-offset-4" style={{ color: COLORS.TEXT_DARK, textDecorationColor: COLORS.PRIMARY }}>
                  {story.buttonText || 'Read Story'}
                </button>
              </div>
            ))}
          </div>
          <div className="mt-10 text-center md:hidden">
            ...
            >
              {pageData.sections[5].mobileViewAllText || 'Read All Stories'}
            </button>
          </div>
        </div>
      </section>
