      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial) => (
            <div
              key={testimonial.id}
              className="bg-white p-7 rounded-xl shadow-md border border-gray-200 hover:shadow-lg transition-all duration-200"
            >
              <p className="text-gray-800 mb-5 text-base leading-relaxed font-medium">{testimonial.message}</p>
              <div>
                <h4 className="font-semibold text-gray-900 text-lg">{testimonial.name}</h4>
                <p className="text-xs text-gray-400 mt-1 tracking-wide uppercase">{testimonial.role}</p>
              </div>
            </div>
          ))}
        </div>
