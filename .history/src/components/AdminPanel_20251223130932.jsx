...
const AdminPanel = () => {
  ...
  const savePage = async () => {
    ...
    setError('');
    try {
      // For home and ilc pages, include title in the update
      const updateData = { sections: pageDoc.sections };
      if ((selectedPage === 'home' || selectedPage === 'ilc') && pageDoc.title) {
        updateData.title = pageDoc.title;
      }

      const updatedPage = await api.updateAdminPage(selectedPage, updateData);
            ...
            {activeSection === 'pages' && selectedPage && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  ...
                  {/* Title Editor for home and ilc pages */}
                  {(selectedPage === 'home' || selectedPage === 'ilc') && (
                    <div className="mb-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-200">
                      <h4 className="text-sm font-semibold text-indigo-900 mb-3">Page Title</h4>
                      <input
                        type="text"
                        value={pageDoc?.title || ''}
                        onChange={(e) => setPageDoc(prev => ({ ...prev, title: e.target.value }))}
                        className="w-full px-4 py-2 border border-indigo-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white"
