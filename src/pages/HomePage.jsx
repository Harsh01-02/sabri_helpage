    ...
    <>
      {pageData.sections.map((section, idx) => {
        switch (section.type) {
          ...
          case 'stats':
            // Find the next section to check if it's a video and render them side by side
            const nextSection = pageData.sections[idx + 1];
            if (nextSection && nextSection.type === 'video') {
              return (
                <section key={idx} className="py-16 bg-white">
                  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                      ...
                      {/* Video section rendered side by side */}
                      <div className="flex flex-col gap-6">
                        <div className="bg-gradient-to-br from-amber-600 to-amber-800 p-4 rounded-3xl shadow-xl transform transition-all hover:shadow-2xl">
                          <div className="relative w-full overflow-hidden rounded-2xl" style={{ aspectRatio: '16/9' }}>
                            <YoutubeEmbed videoId={nextSection.youtubeVideoId} title={nextSection.videoTitle} />
                          </div>
                        </div>
                        <a
                          href={nextSection.channelUrl}
                          target="_blank"
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col">
                              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{nextSection.channelBadge}</span>
                              <span className="text-[17px] font-bold text-gray-900">{nextSection.channelLinkText}</span>
                            </div>
                          </div>
          ...
          case 'video':
            // If video was already rendered with stats, skip rendering it again
            const prevSection = pageData.sections[idx - 1];
            if (prevSection && prevSection.type === 'stats') {
              return null;
            }
            return (
