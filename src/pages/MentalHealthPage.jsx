...
const MentalHealthPage = ({ onNavigate }) => {
  const pageData = usePagesStore((state) => state.getPageBySlug('mental-health'));
  React.useEffect(() => {
    if (pageData) {
      console.log('Mental Health page data:', pageData);
    }
  }, [pageData]);
  return (
    <div className="min-h-screen" style={{ backgroundColor: COLORS.BG_LIGHT_GRAY }}>
      <PageHeader title={pageData?.title || "Mental Health"} subtitle={pageData?.sections?.find(s => s.type === "header")?.subtitle || "Awareness, Support, and Care"} />
          ...
          if (section.type === "hero_image") {
            return (
              <div key={idx} className="mb-8">
                <img
                  src={section.image}
                  alt={section.alt || "Mental Health Support"}
