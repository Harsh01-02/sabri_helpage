...
const PublicationsPage = ({ onNavigate = () => {} }) => {
  ...
  React.useEffect(() => {
    if (pageData) {
      console.log('Publications page data:', pageData);
    }
  }, [pageData]);

  // Extract sections
  const headerSection = pageData?.sections?.find(s => s.type === 'header');
  const articlesSection = pageData?.sections?.find(s => s.type === 'newspaper_articles');
  const digitalSection = pageData?.sections?.find(s => s.type === 'digital_publications');
  ...
  return (
    <section className="py-16 bg-bg-light-gray min-h-screen">
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg px-4 sm:px-8 md:px-16 py-12 flex flex-col gap-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-center text-primary mb-2">{headerSection?.mainHeading || "Glimpse of a Sabri Helpage's Impact"}</h1>
          <h2 className="text-xl md:text-2xl text-center text-gray-600 mb-10 font-normal">{headerSection?.subHeading || 'Publications'}</h2>
        </div>
        {/* Newspaper Articles */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-2">
          {articlesSection?.articles?.map((item, idx) => (
        ...
        <div className="flex justify-center">
          <button className="bg-primary hover:bg-primary-dark text-white font-semibold px-8 py-3 rounded-full shadow transition mb-2">{articlesSection?.buttonText || 'All Media'}</button>
        </div>
        <div>
          <h3 className="text-2xl md:text-3xl text-center text-primary font-bold mb-8 mt-4">{digitalSection?.heading || 'Digital Publications'}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {digitalSection?.items?.map((item, idx) => (
              <div key={idx} className="bg-white rounded-xl shadow border border-gray-100 p-6 flex flex-col items-center">
                ...
                <div className="text-sm text-gray-500 mb-1">{item.date}</div>
                <div className="text-base text-gray-700 font-medium text-center">{item.source}</div>
              </div>
            ))}
          </div>
        </div>
